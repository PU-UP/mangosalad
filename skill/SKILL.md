...
页面新增可选字段 `focus_date`（YYYY-MM-DD 或 null）。这是桌面关注日期，不是 Agent 执行时间，不自动发通知。用户明确指定日期才设置；不依据创建时间或标题擅自推断。近期关注按北京时间包含今天起未来 7 个自然日以及已到期未收起页面；未填日期的页面在全部中显示。更改仍使用带 revision 的 update。桌面拖动顺序独立于页面内容，日常更新不要改桌面排序。
